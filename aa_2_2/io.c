...
static void test_string(char *s, char *word, int* number)
{
	...
	while (1)
	{
		printf("Введите кол-во %s матрицы: ", word);
		fgets(s, SBUF, stdin);
		err = test(s, number);
		if (err && *number > 0)
		{
...
void write_size_of_matr(int *row, int *col)
{
	...
	test_string(s, "строк", row);
	test_string(s, "столбцов", col);
}
...
void write_into_matr(double **matrix, int row, int col)
{
	short fact;
	double *pointer = NULL;
	...
	for (int i = 0; i < row; i++)
	{
		...
		while (1)
		{
			...
			fact = write_into_str(pointer, col);
			if (fact == SUCCESS)
			{
				break;
			}
...
void print_matr(FILE *f, double **matrix, int row, int col)
{
	for (int i = 0; i < row; i++)
	{
		for (int j = 0; j < col; j++)
			fprintf(f, "%5.5f ", matrix[i][j]);
		fprintf(f, "\n");
	}
}
