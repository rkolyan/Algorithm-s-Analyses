...
void find_calculations(int count1, int count2, int threads_count, function_t *functions, const char *filename)
{
	...
	for (int i = 1; i <= threads_count; i <<= 1)
	{
		for (int j = 0; j < count2; j++)
		{
			fill_random_matrix_with_size(count1, count1, matrix1);
			fill_random_matrix_with_size(count1, count1, matrix2);
			clocks += find_time(matrix1, matrix2, matrix_result, count1, count1, count1, threads, threads_count, functions);
		}
		clocks /= count2;
		fprintf(file, "%ld\n", clocks);
	}
...
#define col1 row2
clock_t find_time(double *matr1, double *matr2, double *matr_res, int row1, int row2, int col2, thrd_t *threads, int threads_count, function_t *functions)
{
	clock_t timer;
	//If it is standard
	if (functions->id == 0)
	{
		timer = clock();
		functions->standard_function(matr1, matr2, row1, col1, row2, col2, threads, threads_count, matr_res);
		timer = clock() - timer;
	}
	else //If it is vinograd
	{
		...
		timer = clock();
		functions->vinograd_function(matr1, matr2, row_factor, col_factor, row1, col1, row2, col2, threads, threads_count, matr_res);
		timer = clock() - timer;
		free(row_factor);
		free(col_factor);
	}
